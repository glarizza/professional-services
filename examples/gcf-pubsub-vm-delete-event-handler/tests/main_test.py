...
from main import DnsVmGcApp, EventHandler, RuntimeState, main
...
def handler(app, trigger_event):
    return EventHandler(app=app, data=trigger_event)
...
def test_config_no_project(mock_env_no_proj, app, trigger_event):
    with pytest.raises(EnvironmentError) as err:
        EventHandler(app=app, data=trigger_event)
...
def test_when_warm(app_warm, mock_http, mock_session, trigger_event_done):
    assert RuntimeState.app is app_warm
    main(trigger_event_done, context=None, http=mock_http,
         session=mock_session)
    assert RuntimeState.app is app_warm


def test_when_cold(app_cold, mock_http, mock_session, trigger_event_done):
    assert RuntimeState.app is None
    main(trigger_event_done, context=None, http=mock_http,
         session=mock_session)
    assert RuntimeState.app is not None
...
def test_multiple_calls(app_cold, mock_http, mock_session, trigger_event_done):
    ...
    main(trigger_event_done, context=None, http=mock_http,
         session=mock_session)
    assert RuntimeState.app is not None
    app_memo = RuntimeState.app
    main(trigger_event_done, context=None, http=mock_http,
         session=mock_session)
    assert RuntimeState.app is app_memo
    main(trigger_event_done, context=None, http=mock_http,
         session=mock_session)
    assert RuntimeState.app is app_memo
